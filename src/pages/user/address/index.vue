<template>
    <div>
        ...
        <div class="main">
            <div
                class="list"
                @click="goPage('/address/mod?aid=' + item.aid)"
...
export default {
    ...
    methods: {
        ...
        getAddressData() {
            this.$request(this.$config.baseApi + "/user/address/index?uid=" + this.$store.state.user.uid + "&token=" + this.$config.token).then(res => {
                if (res.code === 200) {
